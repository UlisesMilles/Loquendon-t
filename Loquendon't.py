...
class UnifiedTTSEngine:
    ...
    def synthesize_segment(self, text, voice_index, base_output_path):
        ...
        print(f"   [DEBUG] Chosen Engine: {voice['engine']}, Extension: {ext}") # <-- New debug line
...
def process_and_concatenate(engine: UnifiedTTSEngine, input_file, output_file):
    ...
    for temp_path in temp_files:
        try:
            # FIX: Use from_file() to let pydub/FFmpeg autodetect the file format (.wav or .mp3)
...
def main_menu(engine: UnifiedTTSEngine):
    ...
    while True:
        print("\n--- Python TTS System Menu ---")
        print("1. List available voices (IDs 0 to N-1)")
        ...
        if choice == '1':
            ...
            output_file = input("Enter the output audio file path (e.g., output.mp3 or output.wav): ").strip().strip('"')
...
if __name__ == "__main__":
    ...
    if tts_engine.unified_voices:
        # Set default voice ID for the sample file to the first available voice
        # The user will need to adjust the IDs in the sample file based on their 'List available voices' output.
        # However, for a fully runnable demo, the default voice (ID 0) is sufficient.
        pass
